...
public class TransactionController {
    ...
    public ResponseEntity<Object> get(Principal principal){
        ...
        if (user.getRole().equals("Admin")){
            return transactionService.getAllTransaction(1L, TransactionDto.builder().descriptions("Transaksi E-Money").build());
        }
    ...
    public ResponseEntity<Object> get(@PathVariable Long id) {
        return transactionService.getTransactionById(id, TransactionDto.builder().descriptions("Transaksi Paket Data").build());
    }

    @PostMapping(value = "/new")
    public ResponseEntity<?> create(@RequestBody TransactionDto transaction){
        return transactionService.createNewTransaction(1L, transaction);
    }
